  ...
  for (let i = 1; i <= 10; i++) {
    const lead = await prisma.lead.create({
      data: {
        ...
        status: ["NEW", "REQUIREMENT_DISCUSSION", "CONTACTED", "CONVERTED"][Math.floor(Math.random() * 4)] as any,
        leadSource: "WEBSITE",
  ...
  for (let i = 1; i <= 8; i++) {
    ...
    const project = await prisma.project.create({
      data: {
        ...
        clientId: client.id,
        status: ["COMPLETED", "SCHEDULED", "SHOOT_COMPLETED", "PLANNING"][Math.floor(Math.random() * 4)] as any,
        priority: i % 3 === 0 ? "HIGH" : "MEDIUM",
        category: ["WEDDING", "COMMERCIAL", "OTHER", "PORTRAIT"][Math.floor(Math.random() * 4)] as any,
        startDate: new Date(),
