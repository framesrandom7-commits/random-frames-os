...
export default function ShootFilters() {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const [isPending, startTransition] = useTransition();

  const status = searchParams.get("status") as ShootStatus | "";
  const shootType = searchParams.get("shootType") as ShootType | "";
  ...
  const activeCount = [status, shootType, archived ? "true" : ""].filter(Boolean).length;
    ...
    <Popover>
      ...
      <PopoverContent className="w-80 bg-[#111] border-white/10 text-white p-4" align="end">
        <div className="flex justify-between items-center mb-4">
          <h4 className="font-medium">Filter Shoots</h4>
          {activeCount > 0 && (
